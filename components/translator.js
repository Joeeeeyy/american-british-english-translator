...
class Translator {
    translation(text, locale) {
        ...
        if (locale === "american-to-british") {
             ...
             */
            let regexAmericanToBritishTitle = new RegExp(
                Object.keys(americanToBritishTitles)
                .map(
                    (el) =>
                    el.slice(0, el.indexOf(".")) + "\\" + el.slice(el.indexOf("."))
                )
                .join("|"),
                "gi"
            ); //AmericanToBritishTitle
            let regexAmericanToBritishTime = new RegExp(
                "(\\d{1,2}:\\d{1,2})\\b",
                "gi"
            );
            /**
             ...
             */
            let americanOnlyMatch = Object.keys(americanOnly).filter((el) =>
                text.match(new RegExp(`${el}\\b`, "gi"))
            );
            let spellMatch = Object.keys(americanToBritishSpelling).filter((el) =>
                text.match(new RegExp(`${el}\\b`, "gi"))
            );
            ...
            if (spellMatch) {
                translateString = spellMatch.reduce((acc, curr) => {
                    return acc.replace(
                        new RegExp(`${curr}`, "i"),
                        highlight(americanToBritishSpelling[curr])
                    );
                }, translateString);
            }
            if (titleMatch) {
                translateString = titleMatch.reduce((acc, curr) => {
                    ...
                    let capitalized =
                        ...
                        americanToBritishTitles[curr.toLowerCase()].slice(1);
                    return acc.replace(
                        new RegExp(`${curr}`, "i"),
            ...
            let regexBritishToAmericanTime = new RegExp(
                "(\\d{1,2}.\\d{1,2})\\b",
                "gi"
            ); //time
            ...
            let spellMatch = Object.values(americanToBritishSpelling).filter((el) =>
                text.match(new RegExp(`${el}\\b`, "gi"))
            );
            let titleMatch = Object.values(americanToBritishTitles).filter((el) =>
                text.match(new RegExp(`${el}\\b`, "gi"))
            );
            let britishOnlyMatch = Object.keys(britishOnly).filter((el) =>
                text.match(new RegExp(`${el}\\b`, "gi"))
            );
            ...
            if (spellMatch) {
                translateString = spellMatch.reduce((acc, curr) => {
                    return acc.replace(
                        new RegExp(`${curr}`, "i"),
                        highlight(getKeyByValue(americanToBritishSpelling, curr))
                    );
                }, translateString);
            }
            if (titleMatch) {
                translateString = titleMatch.reduce((acc, curr) => {
                    let title = getKeyByValue(
